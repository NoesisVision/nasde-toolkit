...
### Deep dive — does a public skill, and tuning it, actually help? (Claude Sonnet)

We took a public DDD skill ([Nick Tune's `tactical-ddd`](https://github.com/NTCoding/claude-skillz)) and measured four configurations of Claude Sonnet on two deliberately different tasks — a **feature on a clean DDD codebase** (`ddd-weather-discount`) and a **legacy anemic→rich refactor** (`csharp-movie-rental-anemic`). Each configuration was run repeatedly and each run scored repeatedly, then we took medians (normalized 0–1). Skill activation was verified per run (a mounted skill the agent never invokes scores like no skill at all).
...
|---|---|
| ![Weather radar](examples/ddd-architectural-challenges/assets/radar_weather.png) | ![Movie radar](examples/ddd-architectural-challenges/assets/radar_movie.png) |

The gain is not free. Token usage and run time per configuration:

| Average token usage | Average run time |
|---|---|
| ![Weather tokens](examples/ddd-architectural-challenges/assets/ops_tokens_weather.png) ![Movie tokens](examples/ddd-architectural-challenges/assets/ops_tokens_movie.png) | ![Weather time](examples/ddd-architectural-challenges/assets/ops_time_weather.png) ![Movie time](examples/ddd-architectural-challenges/assets/ops_time_movie.png) |

**Two lessons that generalize:** (1) judge one aggregate number and you miss the story — a real per-dimension gain hides inside an averaged score, which is why we show radars, not a single bar; (2) a skill present on disk is not a skill used — always verify it activated before trusting the result. Charts are generated by [`assets/make_charts.py`](examples/ddd-architectural-challenges/assets/make_charts.py) from the run artifacts.
