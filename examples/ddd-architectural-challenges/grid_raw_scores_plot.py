...
JUDGES = {"claude-fable-5": "Sędzia: Fable 5", "claude-opus-4-8": "Sędzia: Opus 4.8"}
...
def grid_plot(data: dict) -> None:
    ...
    fig.suptitle(
        "Wszystkie pomiary v2.3 — odcień = trial, łącznik = para ocen tego samego sędziego, "
        "większa kropka = dwie identyczne oceny",
    ...
    axes[0].set_ylabel("wynik znormalizowany (v2.3)", fontsize=10, color="#444")
    axes[0].set_ylim(0.5, 1.0)
    fig.tight_layout(rect=(0, 0.02, 1, 0.95))
    out = Path(__file__).with_name("raw_scores_grid_v3.png")
...
def retest_plot(data: dict) -> None:
    ...
    fig.suptitle("Powtarzalność sędziów: ocena #1 vs ocena #2 tego samego triala", fontsize=11.5, color=INK)
    ...
    ax.set_xlabel("niższa z pary ocen", fontsize=10, color="#444")
    ax.set_ylabel("wyższa z pary ocen", fontsize=10, color="#444")
    ax.set_xlim(0.5, 1.0)
    ax.set_ylim(0.5, 1.0)
    ax.set_aspect("equal")
    ...
    for judge, label in JUDGES.items():
        ...
        handles.append(plt.Line2D([], [], marker="o", linestyle="", markersize=8,
                                  ...
                                  label=f"{label}  (śr. |Δ|={mad:.3f}, identyczne: {ties[judge]}/{n_pairs[judge]})"))
    ...
    ax.annotate("punkt na przekątnej = sędzia ocenił identycznie dwa razy",
                (0.98, 0.515), ha="right", fontsize=8.5, color="#777")
    fig.tight_layout(rect=(0, 0, 1, 0.96))
    out = Path(__file__).with_name("judge_retest.png")
    fig.savefig(out, dpi=160, facecolor="white")
    print("saved:", out)
