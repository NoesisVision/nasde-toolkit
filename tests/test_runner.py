...
def test_stage_instruction_overrides_none_without_override_file(tmp_project: ProjectConfig) -> None:
    from nasde_toolkit.runner import stage_instruction_overrides

    variant_dir = tmp_project.project_dir / "variants" / "vanilla"
    assert stage_instruction_overrides(tmp_project, variant_dir, None) == {}


def test_stage_instruction_overrides_swaps_instruction_only(tmp_project: ProjectConfig) -> None:
    from nasde_toolkit.runner import cleanup_instruction_overrides, stage_instruction_overrides

    task_dir = tmp_project.tasks[0].path
    (task_dir / "instruction.md").write_text("canonical instruction")
    (task_dir / "assessment_criteria.md").write_text("rubric v1")

    variant_dir = tmp_project.project_dir / "variants" / "vanilla"
    override_dir = variant_dir / "tasks" / "sample-task"
    override_dir.mkdir(parents=True)
    (override_dir / "instruction.md").write_text("supple hint instruction")

    staged = stage_instruction_overrides(tmp_project, variant_dir, None)
    try:
        assert set(staged) == {"sample-task"}
        staged_dir = staged["sample-task"]["staged"]
        assert (staged_dir / "instruction.md").read_text() == "supple hint instruction"
        assert (staged_dir / "assessment_criteria.md").read_text() == "rubric v1"
        assert (task_dir / "instruction.md").read_text() == "canonical instruction"
        assert staged["sample-task"]["canonical"] == task_dir.resolve()
    finally:
        cleanup_instruction_overrides(staged)
        assert not staged["sample-task"]["staged"].exists()


def test_stage_instruction_overrides_respects_tasks_filter(tmp_project: ProjectConfig) -> None:
    from nasde_toolkit.runner import stage_instruction_overrides
    ...
    override_dir = variant_dir / "tasks" / "sample-task"
    override_dir.mkdir(parents=True)
    (override_dir / "instruction.md").write_text("hint")

    assert stage_instruction_overrides(tmp_project, variant_dir, ["other-task"]) == {}


def test_normalize_instruction_override_trials_rewrites_task_id(tmp_project: ProjectConfig) -> None:
    from nasde_toolkit.runner import (
        cleanup_instruction_overrides,
        normalize_instruction_override_trials,
        stage_instruction_overrides,
    )
    ...
    staged = stage_instruction_overrides(tmp_project, variant_dir, None)
    job_dir = tmp_project.project_dir / "jobs" / "job-x"
    trial_dir = job_dir / "sample-task__abc"
    trial_dir.mkdir(parents=True)
    (trial_dir / "result.json").write_text(
        json.dumps({"task_id": {"path": str(staged["sample-task"]["staged"])}, "trial_name": "sample-task__abc"})
    )
    other_trial = job_dir / "sample-task__keep"
    other_trial.mkdir()
    (other_trial / "result.json").write_text(json.dumps({"task_id": {"path": str(task_dir)}}))

    normalize_instruction_override_trials(job_dir, staged, tmp_project.project_dir)
    cleanup_instruction_overrides(staged)

    rewritten = json.loads((trial_dir / "result.json").read_text())
    assert rewritten["task_id"]["path"] == str(task_dir.resolve())
    assert rewritten["instruction_override"] == "variants/vanilla/tasks/sample-task/instruction.md"
    untouched = json.loads((other_trial / "result.json").read_text())
    assert "instruction_override" not in untouched


def test_build_registry_uses_task_path_overrides(tmp_project: ProjectConfig) -> None:
    from nasde_toolkit.runner import _build_registry

    override = tmp_project.project_dir / "staged-task"
    override.mkdir()
    registry = _build_registry(tmp_project, None, {"sample-task": override})
    assert registry[0]["tasks"][0]["path"] == str(override.resolve())
