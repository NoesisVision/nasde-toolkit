...
PRICING_TOML_EXAMPLE = """\
# Optional per-project model price overrides (USD per 1M tokens).
#
# Precedence (higher wins, merged per model): this file > ~/.nasde/pricing.toml
# > the catalog bundled with nasde. List ONLY the models you want to change or
# add; every other model falls through to the layer below.
#
# The model name MUST match the `model` in your variant.toml (and the model_name
# recorded in each trial's config.json) — otherwise the override is silently
# ignored. Verify with:  nasde pricing show -C . --show-source
# (your model should appear with Layer "project"; if it shows "bundled", the
# name does not match — check for a typo).
#
# Uncomment and edit to activate. Prices use a decimal POINT (2.5), not a comma.
#
...
GITIGNORE_TEMPLATE = """\
jobs/
# Generated by [nasde.plugin]: staged plugin tree inside the build context.
**/_nasde-plugin/
"""
...
def create_project(project_dir: Path, name: str) -> None:
    ...
    _write_if_missing(project_dir / "pricing.toml.example", PRICING_TOML_EXAMPLE)
    ...
    console.print("  pricing.toml.example  (optional model price overrides)")
