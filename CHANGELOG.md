...
- Strip the local-version segment (`+gHASH`) from `hatch-vcs` output so dev
  builds are PyPI-acceptable as pre-releases (`0.4.0.devN` instead of
  `0.4.0.devN+gabcdef`). PEP 440 forbids local versions on the public index. ([#37])
...
[0.4.0]: https://github.com/NoesisVision/nasde-toolkit/compare/v0.3.0...v0.4.0
