...
src/nasde_toolkit/
  ...
  evaluator_backends/
    ...
    claude_subprocess.py   # `Codex -p` subprocess backend (default)
  ...
  skills_installer.py      # `nasde install-skills` — copies bundled authoring skills to ~/.Codex/skills/
  agents/
    __init__.py
    configurable_claude.py # Harbor-compatible Codex agent with sandbox file injection
...
- **All dependencies are core**: `harbor`, `opik` are in `[project.dependencies]`. The evaluator depends on having `Codex` CLI (default) or `codex` CLI installed and authenticated on the host — not bundled. No optional extras — `uv tool install .` gives full functionality. Assessment evaluation is on by default (`--without-eval` to skip).
...
- **Bundled authoring skills**: `.Codex/skills/nasde-benchmark-*` are shipped inside the wheel via `[tool.hatch.build.targets.wheel.force-include]` → `nasde_toolkit/_bundled_skills/`. `nasde install-skills` discovers them via `importlib.resources` (wheel install) with a fallback to the live `.Codex/skills/` when running from an editable / source checkout. `nasde-dev` is deliberately excluded from the bundle (internal dev skill, not for end users).
...
nasde pricing show [OPTIONS]     # Print the effective merged price catalog (ADR-013)
  --show-source                      # Add a Layer column (project/user/bundled) — debug
  -C, --project-dir PATH

nasde init [PROJECT_DIR]         # Scaffold new benchmark project
  -n, --name TEXT

nasde install-skills [OPTIONS]   # Copy bundled Codex authoring skills to a skills dir
  --scope user|project               # Default user: ~/.Codex/skills; project: ./.Codex/skills
...
my-benchmark/
  ...
  pricing.toml                  # Optional: per-project model price overrides (project > ~/.nasde > bundled, ADR-013)
  tasks/
    <task-name>/
  ...
  variants/
    <variant-name>/
      variant.toml              # Required: agent type + model (e.g. agent = "Codex", model = "Codex-sonnet-4-6")
      AGENTS.md                 # Codex instructions (injected into /app/AGENTS.md)
      AGENTS.md                 # Codex instructions (injected into /app/AGENTS.md)
      GEMINI.md                 # Gemini CLI instructions (injected into /app/GEMINI.md)
      skills/                   # Optional: Codex skill snapshots (injected into /app/.Codex/skills/)
        <skill-name>/
...
[evaluation]
backend = "Codex"                            # "Codex" (default) | "codex"
...
[environment]
memory_mb = 4096            # Container memory limit (default: 2048). Codex needs 4096+.
...
[nasde.plugin]              # Optional (ADR-009). Ship a local Codex plugin into the sandbox.
path = "../../../src/plugins/my-plugin"   # dir with .Codex-plugin/plugin.json (required)
...
```toml
agent = "Codex"                    # "Codex" | "codex" | "gemini"
model = "Codex-sonnet-4-6"         # REQUIRED. Model appropriate for the agent family.
...
**`reasoning_effort` (optional)**: controls how hard the model thinks. Priority: `--effort` CLI flag > `variant.toml reasoning_effort` > unset (unset → not passed → Harbor's per-family default; defaults are UNEQUAL across families). Threaded to Harbor via the agent's `reasoning_effort` kwarg, which Harbor turns into the right CLI flag (Codex `--effort`, Codex `-c model_reasoning_effort=`, Gemini ctor arg). **No local validation**: `_resolve_effort` only resolves priority + emptiness, then passes the value straight through. Each layer validates and rejects an unknown level itself — verified empirically: Codex/Gemini via Harbor's CliFlag `choices` (a `ValueError` at agent construction; Gemini also has per-model rules — `minimal` is Flash-only, 2.5 models reject effort entirely), Codex via its own CLI (`Error loading config.toml: unknown variant ...` at startup). A hardcoded per-family allow-list was deliberately removed — effort scales change too often and a stale list wrongly blocks a newly-valid level (it had `codex: low/medium/high`, but Codex actually accepts `none`/`minimal`/`low`/`medium`/`high`/`xhigh`). Typical levels for reference (not enforced): Codex `low`/`medium`/`high`/`xhigh`/`max`, Codex `none`/`minimal`/`low`/`medium`/`high`/`xhigh`, Gemini `minimal`/`low`/`medium`/`high`.
...
commit. **Routing is agent-aware** (ADR-012): for **Codex** the dir is staged
into `/app/.Codex/skills/<name>/` via `plugin_registration.stage_skill_dir`
  ...
  "agents": [
    {
      ...
      "kwargs": {
        "sandbox_files": {
          ...
          "/app/.Codex/skills/my-skill/SKILL.md": "/absolute/path/to/variants/variant-name/skills/my-skill/SKILL.md"
        }
      }
    }
  ]
...
If `harbor_config.json` is absent, `nasde` auto-generates one based on `variant.toml` agent type (`AGENTS.md` → Codex, `AGENTS.md` → Codex, `GEMINI.md` → Gemini).
...
- **Nested Codex sessions**: `Codex` CLI detects `Codex` env var. Runner unsets it before assessment eval.
